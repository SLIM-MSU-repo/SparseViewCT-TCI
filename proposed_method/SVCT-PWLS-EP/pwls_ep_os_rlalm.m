 function [xs, info] = pwls_ep_os_rlalm(x, Ab, yi, R, varargin)
...
%   xold = x;
  ...
  if isempty(arg.xtrue) ~= true
     xroi = embed(x,arg.mask); 
%     figure(20), im(xroi); drawnow
 
     xroi = xroi(:,:,:);
    
     info.RMSE(:,iter) = nrms(col(double(xroi)),col(arg.xtrue));
     disp(['NRMSE = ', num2str(info.RMSE(:,iter))]);  

%      info.SSIM(:,iter)= ssim(xroi, arg.xtrue);
%      disp(['SSIM = ', num2str(info.SSIM(:,iter))]);  
     
%      figure(20), imshow(xroi(:,:,end/2), [800 1200]); 
  end  
...
%   info.relE(:,iter) = norm((xold - x).* arg.roi) / norm(x.* arg.roi);   
